...
// ── Firebase (chỉ dùng Auth để xác thực Google) ──
...
let state = {
  mode: null, selectedParts: [], timeLimit: 0,
  screens: [], currentIdx: 0, answers: {}, flags: {},
  timerInterval: null, secondsLeft: 0,
  started: false, finished: false, startTime: null,
  showSolution: {}, showImg: {},
};
...
function renderSelectPage() {
  ...
  if (history.length > 0) {
    ...
    hw.innerHTML = `
      <span>✓ Lần cuối: <b>${last.totalCorrect}/${last.totalQ} câu đúng</b> — ${date}</span>
    ...
    body.appendChild(hw);
    hw.querySelector('#btnHistory').addEventListener('click', () => showHistoryModal(history));
  }

  body.appendChild(make('div','select-section-label','Chọn part'));

  const PART_INFO = [
    {p:1,range:'Câu 1–6'   },{p:2,range:'Câu 7–31'  },{p:3,range:'Câu 32–70' },
    {p:4,range:'Câu 71–100'},{p:5,range:'Câu 101–130'},{p:6,range:'Câu 131–146'},
  ...
  const fullNote = make('div','exam-full-note','⏱ Thi thử trọn đề: thời gian cố định <b>120 phút</b>.');
  ...
  if (currentUser) {
    const so = make('div','');
    so.style.cssText = 'text-align:center;margin-top:16px';
    so.innerHTML = `
      <span style="font-size:12px;color:#94a3b8">${currentUser.email}</span>
      <button id="btnSignOut" style="margin-left:8px;background:none;border:none;font-size:12px;color:#94a3b8;cursor:pointer;text-decoration:underline">Đăng xuất</button>`;
    body.appendChild(so);
  }
  ...
  // ── Logic ──
  let selParts = [], selMinutes = 0, curMode = 'practice';
  ...
  timeOptions.querySelectorAll('.time-chip').forEach(chip => {
    chip.addEventListener('click', () => {
      ...
      chip.classList.add('selected'); selMinutes=parseInt(chip.dataset.minutes);
      el('customMinutes').value=''; updateUI();
    });
  });
  ...
  el('customMinutes').addEventListener('input', e => {
    ...
    else { if(v>120){v=120;e.target.value=120;} selMinutes=v; }
    timeOptions.querySelectorAll('.time-chip').forEach(c=>c.classList.remove('selected'));
    updateUI();
  });
  ...
  btnStart.addEventListener('click', () => {
    ...
    startExam(curMode, selParts, (isFullTest()?120:selMinutes)*60);
  });

  const btnSignOut = el('btnSignOut');
  if (btnSignOut) btnSignOut.addEventListener('click', async () => {
    await signOut(auth); currentUser=null; renderAuthGate(false);
  });
...
function showHistoryModal(history) {
  ...
  const rows = history.map(r => {
    const date = new Date(r.savedAt).toLocaleString('vi-VN');
    ...
    return `<div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #e2e8f0;font-size:13px">
      <span>${date}</span><span><b>${r.totalCorrect}/${r.totalQ}</b> (${pct}%) — ${r.mode==='practice'?'Luyện tập':'Thi thử'}</span></div>`;
  }).join('');
  overlay.innerHTML = `
...
function renderScreen(idx) {
  ...
  if (sc.type==='p1') {
    ...
    if (isPractice && state.showSolution[sk]) left.appendChild(buildScriptBlock(q.script, q.trans));
    right.appendChild(buildQHeader(q.q,1,sk));
    right.appendChild(buildOptions(q.q,['A','B','C','D']));
  ...
  if (sc.type==='p2') {
    ...
    right.appendChild(buildOptions(q.q,['A','B','C']));
    if (isPractice) right.appendChild(buildSolutionBtn(sk));
  }
  if (sc.type==='p3') {
    const g=sc.group;
    ...
    if (isPractice && state.showSolution[sk]) left.appendChild(buildScriptBlock(g.script, g.trans));
    g.questions.forEach(q=>right.appendChild(buildGroupQBlock(q,3,isPractice,sk)));
    if (isPractice) right.appendChild(buildSolutionBtn(sk));
  }
  if (sc.type==='p4') {
    const g=sc.group;
    ...
    if (isPractice && state.showSolution[sk]) left.appendChild(buildScriptBlock(g.script, g.trans));
    g.questions.forEach(q=>right.appendChild(buildGroupQBlock(q,4,isPractice,sk)));
    if (isPractice) right.appendChild(buildSolutionBtn(sk));
  }
  if (sc.type==='p5') {
    const q=sc.q;
    if (isPractice && state.showSolution[sk] && q.videoUrl) {
      const vw=make('div','video-wrap'); vw.innerHTML=`<iframe src="${youtubeEmbed(q.videoUrl)}" allowfullscreen></iframe>`; left.appendChild(vw);
  ...
  if (sc.type==='p6') {
    ...
    g.questions.forEach(q=>{
      const qw=make('div','q-block'); qw.style.cssText='margin-bottom:24px;padding-bottom:20px;border-bottom:1px solid #e2e8f0';
      qw.appendChild(buildQHeader(q.q,6,sk)); if(q.enQ) qw.appendChild(make('div','q-text',q.enQ));
      qw.appendChild(buildOptions(q.q,['A','B','C','D'])); right.appendChild(qw);
    });
    if (isPractice) right.appendChild(buildSolutionBtn(sk));
  }
  if (sc.type==='p7') {
    ...
    g.questions.forEach(q=>{
      const qw=make('div','q-block'); qw.style.cssText='margin-bottom:24px;padding-bottom:20px;border-bottom:1px solid #e2e8f0';
      qw.appendChild(buildQHeader(q.q,7,sk)); if(q.enQ) qw.appendChild(make('div','q-text',q.enQ));
      qw.appendChild(buildOptions(q.q,['A','B','C','D'])); right.appendChild(qw);
    });
    if (isPractice) right.appendChild(buildSolutionBtn(sk));
  }
  ...
  nextBtn.addEventListener('click',()=>{ state.currentIdx++; renderScreen(state.currentIdx); renderSheet(); saveProgress(); });

  nav.appendChild(prevBtn); nav.appendChild(counter); nav.appendChild(nextBtn);
  screenEl.appendChild(left); screenEl.appendChild(right);
...
function buildMediaLeft(left, g, isPractice, sk, isP7) {
  ...
  if (showVideo) {
    ...
    if (firstQ && firstQ.videoUrl) { const vw=make('div','video-wrap'); vw.innerHTML=`<iframe src="${youtubeEmbed(firstQ.videoUrl)}" allowfullscreen></iframe>`; left.appendChild(vw); }
    ...
    if (isP7 && g.imgs && g.imgs.length>0) {
      ...
      const img=make('img','exam-img'); img.src=g.img; img.alt='Ảnh đề'; left.appendChild(img);
    }
...
function buildGroupQBlock(q, part, isPractice, sk) {
  const wrap=make('div','q-block'); wrap.style.cssText='margin-bottom:24px;padding-bottom:20px;border-bottom:1px solid #e2e8f0';
  wrap.appendChild(buildQHeader(q.q,part,sk));
  if (q.enQ) wrap.appendChild(make('div','q-text',q.enQ));
  if (isPractice && state.showSolution[sk] && q.viQ) wrap.appendChild(make('div','q-text-vi',q.viQ));
  wrap.appendChild(buildOptions(q.q,['A','B','C','D']));
  return wrap;
}

function buildAudioBlock(mp3Url, isPractice, sk) {
  const wrap=make('div','audio-wrap');
  wrap.appendChild(make('div','audio-title','🎧 Audio'));
  const audio=document.createElement('audio');
  audio.id='examAudio_'+sk; audio.src=mp3Url||''; audio.controls=true;
  if (!isPractice) {
  ...
  if (isPractice) {
    ...
    const b3=make('button','audio-ctrl-btn','⏪ -3s'); b3.addEventListener('click',()=>{ audio.currentTime=Math.max(0,audio.currentTime-3); });
    const f3=make('button','audio-ctrl-btn','+3s ⏩'); f3.addEventListener('click',()=>{ audio.currentTime=Math.min(audio.duration||0,audio.currentTime+3); });
    const spd=document.createElement('select'); spd.className='speed-select';
    [0.5,0.75,1,1.25,1.5,1.75,2].forEach(v=>{ const o=document.createElement('option'); o.value=v; o.textContent=`x${v}`; if(v===1) o.selected=true; spd.appendChild(o); });
    spd.addEventListener('change',()=>{ audio.playbackRate=parseFloat(spd.value); });
    ctrl.appendChild(b3); ctrl.appendChild(f3); ctrl.appendChild(spd);
    ctrl.appendChild(make('div','audio-shortcut-hint','← lùi 3s &nbsp;·&nbsp; → tiến 3s &nbsp;·&nbsp; Space tạm dừng/tiếp tục'));
...
function buildScriptBlock(script, trans) {
  ...
  if (script) { const b=make('div','script-box'); b.innerHTML=`<div class="script-label">Script</div>${escHtml(script)}`; frag.appendChild(b); }
  if (trans)  { const b=make('div','script-box vi'); b.innerHTML=`<div class="script-label" style="color:rgba(245,197,24,0.6)">Dịch</div>${escHtml(trans)}`; frag.appendChild(b); }
  return frag;
}
...
function buildQHeader(qNum, part, sk) {
  ...
  flagBtn.addEventListener('click',()=>{ state.flags[qNum]=!state.flags[qNum]; flagBtn.classList.toggle('flagged',!!state.flags[qNum]); renderSheet(); saveProgress(); });
  wrap.appendChild(flagBtn); return wrap;
}

function buildOptions(qNum, letters) {
  const qd=getQuestionData(qNum), opts=qd?(qd.q.enOpts||[]):[];
  ...
  letters.forEach((letter,i)=>{
    ...
    item.addEventListener('click',()=>{
      ...
      list.querySelectorAll('.option-item').forEach(it=>it.classList.toggle('selected',it.dataset.letter===letter));
...
function buildSolutionBtn(sk) {
  const wrap=make('div',''); wrap.style.cssText='display:flex;justify-content:flex-end;margin-top:8px';
  const btn=make('button','btn-solution',state.showSolution[sk]?'🙈 Ẩn bài giải':'💡 Xem bài giải');
  btn.addEventListener('click',()=>{ state.showSolution[sk]=!state.showSolution[sk]; renderScreen(state.currentIdx); });
  ...
  Object.keys(byPart).sort((a,b)=>a-b).forEach(p=>{
    ...
    byPart[p].forEach(({sc,idx})=>{
      ...
      qNums.forEach(qNum=>{
        ...
        else row.addEventListener('click',()=>{ state.currentIdx=idx; renderScreen(state.currentIdx); toggleSheet(false); });
        row.appendChild(make('div','sheet-qnum',String(qNum)));
        const opts=make('div','sheet-opts');
        letters.forEach(lt=>{ const opt=make('div','sheet-opt',lt); if(state.answers[qNum]===lt) opt.classList.add('chosen'); opts.appendChild(opt); });
        ...
        flag.addEventListener('click',e=>{ e.stopPropagation(); state.flags[qNum]=!state.flags[qNum]; flag.classList.toggle('flagged',!!state.flags[qNum]); renderSheet(); saveProgress(); });
...
function submitExam() {
  ...
  const R={
    ...
    totalQ:allQs.length, elapsed, timeLimit:state.timeLimit,
  };
...
function enterReviewMode(R) {
  ...
  if (topRight) topRight.innerHTML=`<span style="font-size:13px;color:rgba(255,255,255,0.7)">✓ Đã nộp — Chế độ xem lại</span>`;
...
onAuthStateChanged(auth, user => {
  ...
  if (isAllowed(user.email)) { currentUser=user; renderSelectPage(); }
  else { signOut(auth).then(()=>renderAuthGate(true)); }
});
